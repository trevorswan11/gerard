...
pub async fn generate_mp3(text: String, output_path: &PathBuf) -> Result<(), Error> {
...
pub async fn tts_command(
    ...
    let voice_channel_id = match ctx.guild_channel().await {
        Some(guild_channel) => {
            ...
            let channel = channels.get(&guild_channel.id)
        ...
        None => {ctx.say("Did not recognize channel id").await?; return Ok(())}
    };

    let output_path = PathBuf::from(format!("./assets/gen/{}-tts.mp3", voice_channel_id.id.to_string()));
